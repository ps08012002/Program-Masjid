...
<div class="max-w-2xl mx-auto bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8 relative">
    ...
    <div class="mb-8 border-b border-gray-50 pb-4">
        ...
        <p class="text-sm text-gray-500 mt-1">
            Perbarui nama masjid, alamat lengkap, beserta wilayah daerah operasionalnya.
        </p>
    </div>

    <!-- Form Utama (Update) -->
    <form action="{{ route('masjid.update', $masjid->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea id="alamat"
                      name="alamat"
                      rows="4"
                      placeholder="Masukkan alamat jalan, RT/RW, dan detail lokasi..."
            ...
            >{{ old('alamat', $masjid->alamat) }}</textarea>
            ...
            <div class="relative">
                ...
                >
                    ...
                    @foreach($daerah as $item)
                        <option value="{{ $item->id }}" {{ old('id_daerah', $masjid->id_daerah) == $item->id ? 'selected' : '' }}>
                            {{ $item->nama }}
                        </option>
                    @endforeach
                </select>
...
<!-- Form Request Delete Tersembunyi -->
<form id="real-delete-form" action="{{ route('masjid.destroy', $masjid->id) }}" method="POST" class="hidden">
    @csrf
    @method('DELETE')
</form>
    ...
    <div class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl border border-gray-100 p-6 transform transition-all scale-95 duration-300 opacity-100 z-10">
        <div class="text-center">
            ...
            <p class="text-sm text-gray-500 mt-2 px-2">
                Apakah Anda yakin ingin menghapus <span class="font-semibold text-gray-800">"{{ $masjid->nama }}"</span>? Seluruh data program bimbingan dan kelas yang tertaut pada masjid ini akan ikut terhapus secara permanen.
            </p>
        </div>
        
        <!-- Pilihan Pilihan Tombol -->
